...
useAppHeader(store, {
  title: 'demo-4-list',
  breadcrumbs: [{ label: 'demo-2' }, { label: 'demo-4-list' }]
})

const data = getMockList(4)
</script>
    ...
    <PskTable :data="data">
      <el-table-column label="Название" sortable prop="name" min-width="350px">
        <template #default="scope">
          <PskLink :to="`./demo-4/${scope.row.uid}`">{{ scope.row.name }}</PskLink>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" width="85">
        <template #default="scope">
          <TableActionsContainer>
            <PskLink small-font-weight :to="`./demo-4/${scope.row.uid}`">Изменить</PskLink>
          </TableActionsContainer>
        </template>
      </el-table-column>
    </PskTable>
